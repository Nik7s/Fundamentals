...
constexpr auto SHARED_MEMORY_SIZE = 1000;
	...
	while (!moribund)
	{
		...
		case WAIT_OBJECT_0:
			...
			else
			{
				...
				(&pCircularBuffer->buffer)[pCircularBuffer->head] = guess;
				printf("Just sent guess: %d\n", guess);
				...
				(&pCircularBuffer->buffer)[pCircularBuffer->head] = player.playerId;
				printf("Just sent player id: %c\n", player.playerId);
