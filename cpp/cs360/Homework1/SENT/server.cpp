...
void createRingBuffer()
{
	...
	while (!moribund)
	{
		...
		case WAIT_OBJECT_0:
			...
			{
				next = (pCircularBuffer->tail + 2) % pCircularBuffer->capacity; // Increment by 2 to account for player's ID
				guess = (&pCircularBuffer->buffer)[pCircularBuffer->tail]; // Read player's ID
				char playerId = (&pCircularBuffer->buffer)[pCircularBuffer->tail + 1]; // Read guess

				// Update the tail;
				pCircularBuffer->tail = next;
				printf("Received guess: %d from Player %c\n", guess, playerId);
